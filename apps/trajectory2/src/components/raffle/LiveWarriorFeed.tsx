      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-4 text-sky-600">
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold text-sunset">{totalCount}</span>
              <span>Warriors Committed</span>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="flex-1">
                <p className="text-sky-700 font-medium mb-1">
                  {maskEmail(latestEntry.email)} joined {getTimeAgo(latestEntry.created_at)}
                </p>
                <p className="text-sky-900 font-bold">
                  <Target className="w-4 h-4 inline mr-2 text-sunset" />
                  Goal: "{latestEntry.transformation_goal}"
                </p>
              </div>
              <Badge className="bg-sunset text-white">
                Entry #{latestEntry.entry_number}
              </Badge>
            </div>
          </motion.div>
        )}
        ...
        <div className="grid gap-4">
          <AnimatePresence mode="popLayout">
            {warriors.slice(1).map((warrior, index) => (
              <motion.div
                key={warrior.id}
