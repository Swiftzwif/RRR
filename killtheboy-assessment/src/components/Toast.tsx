...
export default function Toast({ 
  ...
  return (
    <motion.div
      className={`strata-card p-4 border-l-4 ${getBorderColor()} max-w-sm w-full`}
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-sand-50">
            {title}
          </h4>
          {message && (
        ...
        <button
          onClick={() => onClose(id)}
          className="flex-shrink-0 text-iron-400 hover:text-sand-200 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
